...
grouped <- results %>% group_by(pivot) %>% summarize(
	num_logins=length(event_time),
	num_unique_users=length(unique(user_id)),
	num_successful_logins=length(which(login_status=='Success')),
	num_days=length(unique(day)),
	percent_of_days=num_days/num_days_total
)
