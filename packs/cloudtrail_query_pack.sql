...
FROM (SELECT USER_IDENTITY
     ...
     , coalesce(USERIDENTITY:"userName"
                , user_identity:"sessionContext":"sessionIssuer":"userName"
               ) as roles --Advisory: event times for queries below may or may not be set. Recommended adding event time parameter and running. 
...
WHERE 1=1
  AND cloudtrail.ERRORCODE = 'AccessDenied'
  AND cloudtrail.EVENTSOURCE = 'iam.amazonaws.com'
;
...
SELECT OBJECT_CONSTRUCT('role', roles, 'instance_type', instance_type, 'account_id', recipientAccountId) AS environment
     ...
     , 'WKHSSRZ904' AS query_id
FROM (
SELECT  event_data, EVENTTIME, value:"instanceType" as instance_type, roles, recipientaccountid, eventname
FROM (
  ...
  , object_construct(*) as event_data
  , EVENTTIME, recipientaccountid, eventname
FROM DATA.CLOUDTRAIL
  WHERE EVENTTIME >= '2019-08-14' 
  AND EVENTNAME in ('RunInstances')
), lateral flatten(input=>instance_details)
where value:"cpuOptions":"coreCount" >= 18 or value:"cpuOptions":"threadsPerCore" >2
UNION ALL
SELECT distinct event_data, EVENTTIME, value:"value", roles, recipientaccountid, eventname FROM (SELECT REQUESTPARAMETERS
  , coalesce(USERIDENTITY:"userName"
                , useridentity:"sessionContext":"sessionIssuer":"userName"
               ) as roles 
, object_construct(*) as event_data, EVENTTIME, recipientaccountid, eventname                                                    
FROM DATA.CLOUDTRAIL
  WHERE EVENTTIME >= '2019-08-01' 
  AND EVENTNAME in ('ModifyInstanceAttribute')), lateral flatten(input=>REQUESTPARAMETERS) where path in ('ramdisk', 'instanceType') 
...
FROM (SELECT USERIDENTITY
    ...
    , useridentity
    , EVENTTIME, OBJECT_CONSTRUCT(*) as event_data, eventname
    FROM DATA.CLOUDTRAIL
...
FROM DATA.CLOUDTRAIL cloudtrail
